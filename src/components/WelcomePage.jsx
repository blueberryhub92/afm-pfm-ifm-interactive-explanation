...
import { SlideTracker, TrackedButton } from './shared/SlideTracker';

export const WelcomePage = ({ scroll }) => {
  const [startTime] = useState(Date.now());
  const [moduleHoverTimes, setModuleHoverTimes] = useState({});
  ...
  const modules = [
    {
      ...
      onClick: () => {
        trackModuleClick("python-journey", 1, "Start Python Journey");
        scroll(1);
      },
    ...
    },
    {
      ...
      onClick: () => {
        trackModuleClick("afm-formula", 19, "Additive Factor Model");
        scroll(14);
      },
      size: "normal"
    },
    {
      ...
      onClick: () => {
        trackModuleClick("pfm", 23, "Performance Factor Model");
        scroll(18);
      },
      size: "normal"
    },
    {
      ...
      onClick: () => {
        trackModuleClick("ifm", 26, "Instructional Factor Model");
        scroll(21);
      },
      size: "normal"
    }
  ...
  // Track module clicks with enhanced data
  const trackModuleClick = (moduleId, slideTarget, moduleName) => {
    const timeOnWelcome = Date.now() - startTime;

    trackButtonClick(`module_${moduleId}`, {
      moduleId,
      moduleName,
      slideTarget,
      timeOnWelcomePage: timeOnWelcome,
      modulePosition: modules.findIndex(m => m.id === moduleId),
      totalModules: modules.length
    });

    // Track custom event for module selection
    trackCustomEvent('module_selected', {
      moduleId,
      moduleName,
      selectionTime: timeOnWelcome,
      userChoicePattern: 'welcome_page_selection'
    });
  };

  // Track module hover interactions
  const handleModuleHover = (moduleId, isEntering) => {
    if (isEntering) {
      setModuleHoverTimes(prev => ({
        ...prev,
        [moduleId]: Date.now()
      }));
    } else {
      const hoverStartTime = moduleHoverTimes[moduleId];
      if (hoverStartTime) {
        const hoverDuration = Date.now() - hoverStartTime;

        if (hoverDuration > 200) { // Only track meaningful hovers
          trackHover('module', moduleId, hoverDuration);

          // Track extended hover as interest indicator
          if (hoverDuration > 2000) {
            trackCustomEvent('module_extended_hover', {
              moduleId,
              hoverDuration,
              interestLevel: 'high'
            });
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      trackTimeSpent('welcome_page', timeSpent, 'interactive');

      trackCustomEvent('welcome_page_exit', {
        timeSpent,
        interactionLevel: Object.keys(moduleHoverTimes).length > 0 ? 'interactive' : 'passive'
      });
    };
  }, [startTime, modules, moduleHoverTimes]);
