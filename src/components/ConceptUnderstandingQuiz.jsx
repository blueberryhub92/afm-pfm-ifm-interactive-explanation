...
export const ConceptUnderstandingQuiz = ({ guess2, setGuess2, navigate }) => {
  ...
  const summarizeInteractions = (interactions) => {
    return {
      ...
      totalSubmitAttempts: interactions.filter((i) => i.type === "submit")
        .length,
      averageEditLength:
        interactions
          .filter((i) => i.type === "edit")
          .reduce((sum, i) => sum + i.value.length, 0) /
        Math.max(1, interactions.filter((i) => i.type === "edit").length),
    ...
    trackInputChange(
      ...
      lastInputValue,
      {
        ...
        changeContext: {
          ...
          changeType:
            newValue.length > lastInputValue.length ? "addition" : "deletion",
          timestamp: Date.now(),
        },
      }
    );
