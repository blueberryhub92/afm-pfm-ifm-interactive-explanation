...
const paramMeta = {
  theta: {
    ...
    tooltip: {
      ...
      desc:
        "Range: -3 (low) to +3 (high). Higher θ = higher skill. 0 = average."
    }
  },
  beta: {
    ...
    tooltip: {
      ...
      desc:
        "Range: -2 (easy) to +2 (hard). Higher β = harder tasks. 0 = average."
    }
  },
  gamma: {
    ...
    tooltip: {
      ...
      desc:
        "Range: 0 (slow) to 1 (fast). Higher γ = faster learning."
    }
  },
  practice: {
    label: "T: Practice",
    ...
    tooltip: {
      ...
      desc:
        "How many times practiced. Only increases in-session."
    }
  }
};
...
export const Slide20AFMSimulator = ({scroll}) => {
          ...
          datasets: [
            {
              ...
              pointBorderWidth: labels.map(k => (k === params.practice ? 4 : 0))
            }
          ]
        },
        options: {
  ...
  function retrainModel() {
    ...
    // More responsive ability adjustment
    if (acc >= 0.5) theta = Math.min(3, theta + 0.3);
    else if (acc < 0.2) theta = Math.max(-3, theta - 0.1);
    
    // MUCH more responsive difficulty adjustment - make β change faster
    if (acc > 0.8) {
      // Very high accuracy = tasks too easy, make harder
      beta = Math.min(2, beta + 0.4);
    } else if (acc > 0.6) {
      // Good accuracy = slightly harder tasks
      beta = Math.min(2, beta + 0.2);
    } else if (acc < 0.3) {
      // Low accuracy = tasks too hard, make easier
      beta = Math.max(-2, beta - 0.4);
    } else if (acc < 0.5) {
      // Below average accuracy = make tasks easier
    ...
    if (avgErr > 0.3) {
      ...
      if (acc < 0.5) {
        beta = Math.max(-2, beta - 0.3); // Make easier if struggling
      } else {
        beta = Math.min(2, beta + 0.2); // Make harder if too accurate
      }
    }
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          {meta.icon}
          <h4 className="font-bold text-black text-lg tracking-wide">{meta.tooltip.title}</h4>
        </div>
        ...
        <div className="border-l-8 border-blue-600 bg-blue-100 p-3 rounded-r-lg">
          <div className="text-blue-800 font-bold text-sm tracking-wide">
            CURRENT VALUE: {showTooltip === "gamma" ? params[showTooltip].toFixed(2) : params[showTooltip]}
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="border-4 border-black rounded-xl p-8 bg-white shadow-lg">
            <p className="text-lg text-black leading-relaxed text-center">
              Experience the AFM in action! Adjust parameters, simulate student responses, and watch how the model learns and adapts.
            </p>
          </div>
            ...
            <div className="space-y-8">
              ...
              <div className="border-4 border-black rounded-xl p-8 bg-white shadow-lg">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-xl font-bold text-black uppercase tracking-wide">AFM Formula</h3>
                </div>
                ...
                <div className="bg-gray-50 border-4 border-black rounded-lg p-4 space-y-2 font-mono text-sm">
                  ...
                  <div><strong>Step 4:</strong> P(success) = 1 / (1 + e⁻ˡᵒᵍⁱᵗ)</div>
                  <div className="text-lg font-bold text-blue-600 mt-3">
                    <strong>Result:</strong> P(success) = {(prob * 100).toFixed(1)}%
                  </div>
                </div>
              </div>
              ...
              <div className="border-4 border-black rounded-xl p-8 bg-white shadow-lg">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-xl font-bold text-black uppercase tracking-wide">Model Parameters</h3>
                </div>
                  ...
                  {Object.entries(paramMeta).map(([key, meta]) => (
                    <div key={key} className="space-y-3">
                      ...
                      >
                        {meta.icon} 
                        {meta.label}
                        <span className="text-sm font-normal text-gray-600">(click for info)</span>
                      </label>
                      ...
                      <div className="flex items-center gap-4">
                        <input
                          ...
                          className={`flex-1 h-3 bg-gray-200 rounded-lg appearance-none cursor-pointer border-2 border-black ${
                            !sessionActive && key !== "practice" ? 'opacity-50' : ''
                          }`}
                        />
                        <span className={`font-mono w-16 text-center text-${meta.color}-800 font-bold text-lg border-2 border-black rounded px-2 py-1 bg-gray-50`}>
                ...
                <div className="mt-8 p-6 border-4 border-blue-600 rounded-xl bg-blue-50">
                  <div className="text-center mb-4">
                    <span className="text-black font-bold text-lg">SUCCESS PROBABILITY</span>
                  </div>
            ...
            <div className="space-y-8">
              ...
              <div className="border-4 border-black rounded-xl p-8 bg-white shadow-lg">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-xl font-bold text-black uppercase tracking-wide">Learning Curve</h3>
                </div>
                ...
                <div className="mt-4 p-4 border-4 border-green-600 rounded-lg bg-green-50 text-center">
                  <div className="font-bold text-green-800">
                    CURRENT: Practice {params.practice} | Probability {(prob * 100).toFixed(1)}%
                  </div>
                </div>
              </div>
              ...
              <div className="border-4 border-black rounded-xl p-8 bg-white shadow-lg">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                  <button
                    ...
                    className={`px-4 py-3 bg-green-600 text-white border-4 border-black rounded-xl font-bold uppercase tracking-wide transition-all transform hover:scale-105 ${
                      !sessionActive ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white hover:text-green-600'
                    }`}
                  >
                    ✓ Correct
                  </button>
                  ...
                  <button
                    ...
                    className={`px-4 py-3 bg-red-600 text-white border-4 border-black rounded-xl font-bold uppercase tracking-wide transition-all transform hover:scale-105 ${
                      !sessionActive ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white hover:text-red-600'
                    }`}
                  >
                    ✗ Incorrect
                  </button>
                ...
                {(!sessionActive && retrainingData.length > 0) && (
                  <div className="border-4 border-yellow-600 rounded-xl p-6 bg-yellow-100 mb-6">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <h4 className="font-bold text-yellow-800 text-lg uppercase">Retraining Ready</h4>
                    </div>
                ...
                <div className="border-l-8 border-purple-600 bg-purple-100 rounded-r-xl p-4">
                  <h4 className="font-bold text-purple-800 mb-3 text-lg uppercase">Recent Responses</h4>
                        ...
                        .map((entry, i) => (
                          ...
                          >
                            <div className="font-bold">
                              Practice {entry.practice}: {entry.correct ? "✓ Correct" : "✗ Incorrect"}
                            </div>
          ...
          <div className="border-4 border-black rounded-xl p-8 bg-gradient-to-r from-yellow-100 to-orange-100 shadow-lg">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-2xl font-bold text-black uppercase tracking-wide">AFM: What Happens When?</h3>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="border-4 border-blue-600 rounded-xl p-6 bg-blue-50">
                <h4 className="font-bold text-blue-800 mb-4 text-xl uppercase tracking-wide">During Session</h4>
              ...
              <div className="border-4 border-purple-600 rounded-xl p-6 bg-purple-50">
                <h4 className="font-bold text-purple-800 mb-4 text-xl uppercase tracking-wide">During Retrain</h4>
                <ul className="space-y-2 text-black font-bold">
                  <li>• θ, β, γ updated based on performance</li>
                  <li>• Past responses are used for tuning</li>
                  <li>• β now changes much more aggressively!</li>
